...
var buildIncludeList = function(config) {
    ...
    if (config.paths && config.paths.include) {
        for (i = 0; i < config.paths.include.length; ++i) {
            result.push('/I"' + config.paths.include[i] + '"');
        }
    }
...
var buildCompileIdeSourceFileAction = function(options, project, configuration, path) {
    ...
    if (configuration.pch && configuration.pch.source &&
        require('path').resolve(configuration.pch.source) === path) {
        return '';
    }
...
var buildLinkApplicationAction = function(options, project, configuration, platform) {
    ...
    result.push('/NOLOGO');
    result.push('"kernel32.lib" "user32.lib" "gdi32.lib" "winspool.lib" "comdlg32.lib" ');
    result.push('"advapi32.lib" "shell32.lib"');
    result.push('"ole32.lib" "oleaut32.lib" "uuid.lib" "odbc32.lib" "odbccp32.lib"');
    result.push('/MANIFEST');
    result.push('/ManifestFile:"Release\direct2d-example.exe.intermediate.manifest"');
    result.push('/ALLOWISOLATION');
    result.push('/MANIFESTUAC:"level=\'asInvoker\' uiAccess=\'false\'"');
    result.push('/DEBUG');
    result.push('/PDB:"bin\Release\direct2d-example.pdb"');
    result.push('/SUBSYSTEM:WINDOWS');
    result.push('/OPT:REF');
    result.push('/OPT:ICF');
    result.push('/PGD:"bin\Release\direct2d-example.pgd"');
    result.push('/LTCG');
    result.push('/TLBID:1');
...
var buildLinkDynamicAction = function(options, project, configuration, platform) {
    var result = [];
...
var buildLinkAction = function(options, project, configuration, platform) {
    ...
    switch (project.type) {
        case 'application':
            result = result.concat(buildLinkApplicationAction(options, project, configuration, platform));
            break;

        case 'dynamic-library':
            result = result.concat(buildLinkDynamicAction(options, project, configuration, platform));
            break;
...
var initialize = function(options) {
    ...
    for (i = 0; i < options.config.projects.length; ++i) {
        ...
        if (!project.configurations) {
            throw "The project " + project.name + " does not have any configurations.";
        }
        ...
        for (j = 0; j < project.configurations.length; ++j) {
            ...
            if (!fs.existsSync(intermediateDir)) {
                require('mkdirp').mkdirp(intermediateDir, 0777);
            }
        }
    }
