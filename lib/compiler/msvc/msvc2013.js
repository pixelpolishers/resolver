...
function Class(application) {
    ...
    var registerObjectFile = function(configuration, filePath) {
        ...
        objFile = process.cwd() + '\\' + configuration.getIntermediateDirectory() + '\\' + fileName + '.obj';
        objectFiles.push(objFile);
    };

    var buildLinkManifest = function(configuration) {
        var result = [], outputPath, outputExt;

        outputExt = configuration.getParsedExtension();
        outputPath = process.cwd() + '\\' + variables.parse(configuration.getOutputPath()) + outputExt;
    ...
    var buildLinkCommands = function(project, configuration, isDLL) {
        var result = ['link'], outputPath, outputDir, outputExt, intDir, vendorDir, dependencies,
            dependencyDir, dependencyConf;
        ...
        intDir = process.cwd() + '\\' + variables.parse(configuration.getIntermediateDirectory());
        ...
        vendorDir = application.getConfig().getVendorDirectory();
        configuration.getDependencies().forEach(function(dependency) {
            dependencyDir = vendorDir + '/' + dependency.getName();
            dependencyConf = application.loadDependencyConfig(dependency.getName());

            dependencyConf.getProjects().forEach(function(project) {
                ...
                var cfgName = dependency.getConfig() || configuration.getName();
                project.getConfigurations().forEach(function(projConf) {
                    if (projConf.getName() === cfgName) {
                        var depOutputExt = projConf.getParsedExtension();
                        var depOutputPath = variables.parse(projConf.getOutputPath());
                        var projDir = require('path').resolve(dependencyDir + '/' + depOutputPath + depOutputExt);
                        result.push(projDir);
                    }
                });
    ...
    var buildLinkApplicationAction = function(project, configuration) {
        return buildLinkCommands(project, configuration, false);
    };

    var buildLinkDynamicAction = function(project, configuration) {
        return buildLinkCommands(project, configuration, true);
    };

    var buildLinkStaticAction = function(project, configuration) {
        var result = ['lib'], outputPath, outputDir, outputExt, intDir;
    ...
    var registerDef = function(definitions, name) {
        if (definitions.indexOf(name) === -1) {
            definitions.push(name);
        }
    };

    var buildDefineList = function(configuration) {
        var configType = configuration.getProject().getType(),
            definitions = configuration.getDefinitions();

        registerDef(definitions, 'WIN32');

        if (configType === 'application' || configType === 'dynamic-library') {
            registerDef(definitions, '_WINDOWS');
        }

        if (configType === 'application') {
            registerDef(definitions, '_MBCS');
        }

        if (configuration.getDebug()) {
            registerDef(definitions, '_DEBUG');
        } else {
            registerDef(definitions, 'NDEBUG');
        }

        if (configuration.getProject().getType() === 'static-library') {
            registerDef(definitions, '_LIB');
        }

        if (configuration.getProject().getType() === 'dynamic-library') {
            registerDef(definitions, '_USRDLL');
            registerDef(definitions, 'DLLTEST_EXPORTS');
            registerDef(definitions, '_WINDLL');
        }

        registerDef(definitions, 'UNICODE');
        registerDef(definitions, '_UNICODE');
    ...
    var buildIncludeList = function(configuration) {
        var i, result = [], paths, dependencies, dependencyConf, vendorDir, dependencyDir;

        // Use the project's include paths:
        paths = paths = configuration.getProject().getPaths().getIncludePaths();
        for (i = 0; i < paths.length; ++i) {
            result.push('/I"' + require('path').resolve(paths[i]) + '"');
        }

        // Use the configuration's include paths:
        paths = paths = configuration.getPaths().getIncludePaths();
        for (i = 0; i < paths.length; ++i) {
            result.push('/I"' + require('path').resolve(paths[i]) + '"');
        }

        // Add the dependency include paths:
        vendorDir = application.getConfig().getVendorDirectory();
        dependencies = configuration.getDependencies();
        for (i = 0; i < dependencies.length; ++i) {
            dependencyDir = vendorDir + '/' + dependencies[i].getName();
            dependencyConf = application.loadDependencyConfig(dependencies[i].getName());

            // Iterate through all the projects and add the include paths:
            var projects = dependencyConf.getProjects();
            for (var projectIdx = 0; projectIdx < projects.length; ++projectIdx) {
                var paths = projects[projectIdx].getPaths();
                var includePaths = paths.getIncludePaths();
                for (var incIdx = 0; incIdx < includePaths.length; ++incIdx) {
                    result.push('/I"' + require('path').resolve(dependencyDir + '/' + includePaths[incIdx]) + '"');
                }
            }
        }
    ...
    var buildIntermediateArgument = function(configuration) {
        var intermediateDir = process.cwd() + '\\' + variables.parse(configuration.getIntermediateDirectory());
    ...
    var buildPchArguments = function(project, configuration, use) {
        var result = [], pchName, pch, intDir = process.cwd() + '\\' + configuration.getIntermediateDirectory();
    ...
    var buildIdeProjectPlatformAction = function(project, configuration) {
        ...
        intDir = process.cwd() + '\\' + configuration.getIntermediateDirectory();
        ...
        if (pch) {
            ...
            cmd.push(require('path').resolve(pch.getSource()));
    ...
    var buildIdeProjectConfigurationAction = function(project, configuration) {
        ...
        var intDir = process.cwd() + '\\' + variables.parse(configuration.getIntermediateDirectory());
    ...
    var buildIdeProjectAction = function(project) {
        ...
        result.push('rem Parsing project ' + project.getName() + ' of type ' + project.getType());
        result.push('');

        for (var i = 0; i < configurations.length; ++i) {
            variables.pushIdeConfiguration(configurations[i]);

            result.push('rem Parsing configuration ' + configurations[i].getName());
    ...
    var buildIdeDependencyAction = function(dependencyName) {
        var result = [],
            vendorDir = application.getConfig().getVendorDirectory(),
            depdencyDir = vendorDir + '/' + dependencyName,
            cwdBackup = process.cwd();

        // If the dependency does not exists, ignore:
        if (!require('fs').existsSync(depdencyDir)) {
            return result;
        }

        // Change the current working directory to the directory of the dependency:
        process.chdir(depdencyDir);

        // Load the project configuration:
        var depdencyConfig = application.getConfigReader().read('resolver.json');
        result = result.concat(buildCompileProjectsCommands(depdencyConfig.getProjects()));

        // We're done, change back:
        process.chdir(cwdBackup);

        return result;
    };

    var buildCompileProjectsCommands = function(projects) {
        var content = [];
    ...
    var executeCompileCommand = function(lines, callback) {
        var content = [];
    ...
    this.compileProjects = function(callback) {
        var projects = application.getConfig().getProjects();

        var content = buildCompileProjectsCommands(projects);

        executeCompileCommand(content, callback);
    };

    this.compileDependencies = function(callback) {
        var content = [], dependencies = application.getLockFile().getDependencies();

        for (var name in dependencies) {
            content = content.concat(buildIdeDependencyAction(name));
        }
