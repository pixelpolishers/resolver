function Class() {
    var DependencyClass = require('../ide/dependency').Class;
    ...
    function loadIdeSource(json) {
        ...
        if (json.sources) {
            for (var i = 0; i < json.sources.length; ++i) {
                var src = loadIdeSource(json.sources[i]);
    ...
    function loadIdePaths(result, json) {
        ...
        if (json && json.exclude) {
            for (var i = 0; i < json.exclude.length; ++i) {
                paths.addExcludePath(json.exclude[i]);
            }
        }

        if (json && json.executable) {
            for (var i = 0; i < json.executable.length; ++i) {
                paths.addExecutablePath(json.executable[i]);
            }
        }

        if (json && json.include) {
        ...
        if (json && json.library) {
            for (var i = 0; i < json.library.length; ++i) {
                paths.addLibraryPath(json.library[i]);
            }
        }

        if (json && json.reference) {
            for (var i = 0; i < json.reference.length; ++i) {
                paths.addReferencePath(json.reference[i]);
            }
        }

        if (json && json.source) {
            for (var i = 0; i < json.source.length; ++i) {
                paths.addSourcePath(json.source[i]);
            }
        }
    ...
    function loadIdeConfigDependencyString(result, name) {
        var dependency = new DependencyClass();

        dependency.setName(name);
        dependency.setConfig(result.getName());

        result.addDependency(dependency);
    }

    function loadIdeConfigDependencyObject(result, json) {
        if (!json.name) {
            throw 'A name is needed to define which dependency is meant.';
        }

        var dependency = new DependencyClass();

        dependency.setName(json.name);
        dependency.setConfig(json.config || result.getName());
        dependency.setProject(json.project || undefined);
        dependency.setDynamic(!!json.dynamic);

        result.addDependency(dependency);
    }

    function loadIdeConfigDependency(result, json) {
        if (require('util').isArray(json)) {
            for (var i = 0; i < json.length; ++i) {
                loadIdeConfigDependency(result, json[i]);
            }
        } else if (typeof(json) === 'string') {
            loadIdeConfigDependencyString(result, json);
        } else if (typeof(json) === 'object') {
            loadIdeConfigDependencyObject(result, json);
        } else {
            throw 'Invalid dependency type provided: ' + typeof(json);
        }
    }
    ...
    function loadIdeConfig(project, json) {
        ...
        if (!json.output) {
            throw 'Your IDE project configuration has no output path.';
        }
        ...
        result.setName(json.name);
        result.setOutputPath(json.output);
        ...
        if (json['output-extension']) {
            result.setOutputExtension(json['output-extension']);
        }
        ...
        if (json.dependencies) {
            loadIdeConfigDependency(result, json.dependencies);
        }
    ...
    function loadIdeProject(json) {
        ...
        if (json.dependencies) {
            ...
            for (var i = 0; i < json.dependencies.length; ++i) {
                var name = json.dependencies[i].name, dependency = new DependencyClass;
                
                dependency.setName(name);
                dependency.setVersion(json.dependencies[i].version);
    ...
    function loadConfig(json) {
        ...
        result.setLicense(json.license || '');
    ...
    this.read = function(path) {
        try {
            var fileJson = loadJson(path);

            return loadConfig(fileJson);
        } catch (e) {
