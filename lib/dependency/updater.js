...
function Class(application) {
    ...
    var lockerDependencies = application.getLockFile().getDependencies();

    var downloadDependency = function(dependency, callback) {
        var driver, DriverClass, outputPath;

        switch (dependency.package.repository.type) {
            case 'git':
            case 'github':
    ...
    var ignoreDependency = function(dependency, callback) {
        logger.log('Ignoring non-existing dependency "' + dependency + '"...');
    ...
    var createDependencyCommands = function(container, dependencies) {
        ...
        // Ignore all dependencies that don't exist:
        ...
        for (var dependencyName in lockerDependencies) {
            if (!hasDependency(dependencies, dependencyName)) {
                commands.push({
                    'type': 'remove',
    ...
    var executeCommands = function(commands, callback) {
        ...
        for (var i = 0; i < commands.length; ++i) {
            switch (commands[i].type) {
                ...
                case 'ignore':
                    ignoreDependency(commands[i].dependency, onActionCompleted);
                    break;
