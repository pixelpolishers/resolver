function Class(generator, project) {
    ...
    var writePropertyGroup = function(configuration) {
        var element, outputPath, outputExt, outDir, intDir, projectDir;
        ...
        if (configuration.getOutputExtension()) {
            outputExt = '.' + configuration.getOutputExtension();
        } else if (configuration.getPlatform() === 'linux') {
        ...
        outputPath = variables.parse(configuration.getOutputPath()) + outputExt;
        ...
        configuration.getDependencies().forEach(function(configDep) {
            ...
            depConfigFile.getProjects().forEach(function(project) {
                ...
                project.getConfigurations().forEach(function(projConf) {
                    ...
                    var outputPath = application.getDependencyDirectory(configDep.getName())
                            + '/' + variables.parse(projConf.getOutputPath()) + '.lib';

                    dependencies.push(pathLib.resolve(outputPath));
