/**
 * Counts the amount of members that the given object has.
 *
...
module.exports.getFilesFromDirectory = function(dirPath) {
    var i, result = [], stats, filePath, fileNames = [];

    try {
        fileNames = require('fs').readdirSync(dirPath);
    } catch (e) {
    }
...
module.exports.httpGetRequest = function(options, httpCallback) {
    ...
    request.on('error', function(e) {
        require('./logger').error('Problem with request: ' + e.message);
        httpCallback(null);
    });
