...
git remote add deploy git@github.com:pixelpolishers/resolver.git
git fetch deploy
...
# file conflicts:
mv resolver.phar resolver.phar.tmp

# Checkout gh-pages and add PHAR file and version:
git checkout -b gh-pages deploy/gh-pages
mv resolver.phar.tmp resolver.phar
sha1sum resolver.phar > resolver.phar.version
git add resolver.phar resolver.phar.version

# Commit and push:
git commit -m 'Rebuilt resolver.phar'
